...
def input_overtime():
    ...
    try:
        ...
        if tahun in my_data:
            if bulan in my_data.get(tahun):
                if tanggal in my_data.get(tahun).get(bulan):
    ...
    if calculation.cek_data_isi():
        ...
        try:
            ...
            if tgl == f_yyyy:
                ...
                if (f"{yyyy}" in my_data and
                        f"{mm}" in my_data[yyyy] and
                        f"{dd}" in my_data[yyyy][mm]):
